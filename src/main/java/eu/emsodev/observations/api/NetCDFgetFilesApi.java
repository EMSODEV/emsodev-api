...
public interface NetCDFgetFilesApi {
    ...
    @ApiOperation(value = "It represents the Time Series retrieved as NETCDF", notes = "Get NetCDF file of a specific Observatory`.", response = File.class, tags={ "Observations Time-series as NetCDF", })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "Time series list.", response = File.class) })
    @RequestMapping(value = "/fileasnetcdf",
        produces = { "application/json" }, 
        method = RequestMethod.GET)
    ResponseEntity<File> filesGet();
